...
def get_objective_function(model_name: str, X: pd.DataFrame, y: pd.Series):
    def objective(trial: optuna.Trial) -> float:
        if model_name == "RandomForest":
            ...
            model_instance = settings.MODELS_TO_TUNE[model_name](
                random_state=42, probability=True, **params
            )
...
def train_single_model(model_name: str, X: pd.DataFrame, y: pd.Series) -> TrainResult:
    study = optuna.create_study(direction="maximize")
    objective_fn = get_objective_function(model_name, X, y)
    study.optimize(objective_fn, n_trials=50, n_jobs=1, show_progress_bar=False)

    best_params = study.best_params
    logger.info(
        f"[Optuna] Otimização para {model_name} concluída. Melhores parâmetros: {best_params}"
    )
...
def main() -> None:
    ...
    logger.info("="*50)
    logger.info(
        "Aplicando engenharia de features robusta..."
    )
    
    fe_pipeline = Pipeline(steps=[
        ('feature_engineering', FunctionTransformer(advanced_feature_engineering)),
    ])
    ...
    with ThreadPoolExecutor(max_workers=os.cpu_count()) as executor:
        futures = {
            executor.submit(train_single_model, name, X_processed.copy(), y.copy()): name
            for name in settings.MODELS_TO_TUNE
        }
    ...
    stacking_estimators = [
        (name.lower(), model) for name, model, _, _, _ in top_models
    ]
    ...
    training_metrics["ensemble_model"] = {
        ...
        "accuracy": mean_score,
        "std_dev": std_score,
        "estimators": [name for name, _, _, _, _ in top_models],
